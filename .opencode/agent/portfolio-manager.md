...
4. **Guardrails.** Run `from finance.risk_guardrails import check_circuit_breakers` on the portfolio metrics with config `{"max_position_pct": 25.0, "max_drawdown_pct": 20.0}`. Report `halt`, `blocked_reasons`, and per-check booleans clearly. Any HALT must be reported first and prominently.
5. **Rebalance proposal.** For drift beyond ~2 percentage points, propose rebalance actions (sell/buy by symbol, approximate dollar amounts and share counts) that would bring weights to target. Respect the 25% single-position cap: never propose adding to a position that would exceed it. Note that the user contributes $500–$1000/month, so small drift can be fixed by redirecting new contributions instead of selling.
